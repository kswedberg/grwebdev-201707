...
bs.watch([
  'app/index.html',
  'app/examples/**/*'
], {cwd}, function(event, file) {
  if (event !== 'change') {
    return;
  }

  let isExample = file.startsWith('app/examples');
  let firstTask = isExample ? copy.copyFile(file) : Promise.resolve();
  ...
  firstTask.then(() => {
    ...
    return copy.buildIndex();
  })
  .then(() => {
    console.log(chalk.green('Rebuilt index.html'));
    bs.reload();
  })
  .catch(console.error);
});
